---
title: "Western Skink Metadata and Quality Assessment Standard"
...
outputs: "PLSK_ModelQualityAssessment.csv"
...
#[6] Five letter wildlife codes; generally consisting of class code (‘F’ = fish, ‘A’ = amphibians, ‘R’ = reptiles, ‘M’ = mammals) plus four letters derived from either scientific names or from English names (birds). E.g., ‘A-ASTR’ = Amphibian – Ascaphus truei. See https://ecoreserves.bc.ca/wp-content/uploads/2014/06/BC-Vertebrates.pdf for comprehensive list. 
Species_code<-"R-PLSK"
...
Global_ranking<-"G5 (2005)"

#[9] Categorical: Year-round resident, Seasonal breeder, Seasonal resident, Transient migrant
Distribution_status<- "Year-round resident"

#[10] Categorical: FD, SH, TH, ST, FS, FT, FST
Life_requisite<-"FST"

#[11] Categorical: CO, HI, LIA, LIG, LIP, LIPE, LIPL, LIS, LIW, LIWE, LIWL, MS, RB, RE, SG
Life_stage<-"LIA"
...
kable(SHMPlanning_tab, caption = "Table 1: SHM Planning")
...
Species_source <- c("Global Biodiversity Information Facility (GBIF)", "Conservation Data Centre (CDC)","Wildlife Species Index - Survey Occurrences (WSI_SO)")
...
Access_date <- c("02-23-2023", "01-03-2023", "01-03-2023")
...
#Number_of_records <- c(nrow(PLSK_gbif), nrow(PLSK_EO),nrow(PLSK_WSI_SO))
Number_of_records <- c("61","37","1322")

#[22] Categorical: Citizen science, Targeted surveys, Expert knowledge, Various
Collection_method <- c("Various", "Various", "Targeted survey")

#[23] Categorical: points, lines, polygons, various
Spatial_format <- c("points", "polygons", "polygons")
...
Preparation <- c("Filtered by scientific species name and range in BC range was extracted from provincial range map extent", "WSI data were not filtered, though excludes Incidental Observations", "EO data were not filtered")
```
...
kable(SHMSpeciesInput_tab, caption = "Table 2: Species Data Inputs")
...
Total_records_preclean <- "1420"

#[27] DD-MM-YYYY; Earliest date of species observations. 
# min(PLSK_WSI_SO$OBSERVATION_YEAR)
# min(PLSK_WSI_SO$OBSERVATION_MONTH)
# min(PLSK_WSI_SO$OBSERVATION_DAY)
# 
# min(PLSK_EO$FIRST_OBS)


Date_range_min<- "05-01-1996"

#[27] DD-MM-YYYY; Latest date of species observations.
# max(PLSK_WSI_SO$OBSERVATION_YEAR)
# max(PLSK_WSI_SO$OBSERVATION_MONTH)
# max(PLSK_WSI_SO$OBSERVATION_DAY)
# 
# max(PLSK_EO$FIRST_OBS)

Date_range_max<- "09-31-2019"
...
#Final_records_postclean <- sum(PLSK_points_clean$OBSERVED_NUMBER)
Final_records_postclean <- "1391"
...
kable(SHMMasterSpeciesInput_tab, caption = "Table 3: Species Presence & Absence Dataset")
...
Predictor_short_nm <- as_tibble(list.files(path = "R:/22.0253_SHM_Year2/Worked_Examples/Western_Skink/Data/Predictor_Inputs/Processed", pattern = '_noNA.tif$', all.files = TRUE, full.names=FALSE))
...
str_detect(str_to_lower(Predictor_short_nm$value),"lc|vri|dist|bec|tree") ~ "landcover",
  TRUE ~ NA_character_))
Predictor_class<-Predictor_class$value
...
Predictor_full_nm<- c("Towns and cities", "Biogeoclimatic_zones", "Bedrock_intrusive_rock",     "Bedrock_metamorphic_rock", "Bedrock_sedimentary_rock", "Bedrock_ultramafic_rock", "Bedrock_volcanic_rock",
"Cutblock_features", "Annual_mean_precitpitation", "Annual_mean_temperature", "Mean_annual_relative_humidity", "Summer_heat_moisture_index", "Fall_mean_temperature", "Summer_mean_temperature", "Spring_mean_temperature", "Winter_mean_temperature", "Elevation", "Road_features", "Water_features", "Fire_features", "Herbaceous", "Coniferous", "Broadleaf", "Mixedwood", "Snow_ice", "Rock_rubble", "Exposed_barren", "Shrubs", "Wetlands", "Wetland_treed", "None", "Betula_papyrifera", "Larix_occidentalis", "Picea_engelmannii", "Pinus_contorta", "Populus_tremuloides", "Abies_lasiocarpa", "Pseudotsuga_menziesii", "Thuja_plicata", "Tsuga_heterophylla", "Tsuga_mertensiana", "Diurnal_anisotropic_heat", "Terrain_ruggedness_index", "Slope", "Soil_till", "Soil_rock", "Soil_water", "Soil_colluvium", "Soil_glaciofluvial", "Forest_age", "Forest_crown_closure", "Herb_cover", "Shrub_cover")
...
Description <- c("Distance to nearest town or city", "Category codes", "percent area within 100m radius",     "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "Distance to nearest cutblock age <30yrs of 2005", "Climate normal 1991-2020", "Climate normal 1991-2020", "Climate normal 1991-2020", "Climate normal 1991-2020", "Climate normal 1991-2020", "Climate normal 1991-2020", "Climate normal 1991-2020", "Climate normal 1991-2020", "Average elevation within grid cell", "Distance to nearest road", "Distance to stream, river, lake, wetland", "Distance to nearest 1995-2005 fire", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "Function of aspect and slope; sun exposure", "Function of elevation", "Average slope within grid cell", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "percent area within 100m radius", "Average age within grid cell", "Average percent cover within grid cell", "Average percent cover within grid cell", "Average percent cover within grid cell")

#[40] Specify ‘continuous’ or ‘categorical’, in order listed in [36], separated by commas.
Predictor_type <- c("Continuous", "Categorical", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous", "Continuous")
...
Predictor_source<- c("Administration Areas; DataBC", "BECWeb version 12; DataBC", "BC-DigitalGeo; DataBC",     "BC-DigitalGeo; DataBC", "BC-DigitalGeo; DataBC", "BC-DigitalGeo; DataBC", "BC-DigitalGeo; DataBC", "Consolidated Cutblocks; DataBC", "Climate BC", "Climate BC", "Climate BC", "Climate BC", "Climate BC", "Climate BC", "Climate BC", "Climate BC", "Digital Elevation Model; DataBC", "Digital Road Atlas; DataBC", "Fresh Water Atlas; DataBC", "CA_Forest_Fire_1985-2020; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_VLCE2_2009; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "CA_forest_lead_tree_species; National Terrestrial Ecosystem Monitoring System", "SAGA derivative of Digital Elevation Model", "SAGA derivative of Digital Elevation Model", "Digital Elevation Model; DataBC", "Soil Parent Materials; TEI Group", "Soil Parent Materials; TEI Group", "Soil Parent Materials; TEI Group", "Soil Parent Materials; TEI Group", "Soil Parent Materials; TEI Group", "Vegetation Resources Inventories", "Vegetation Resources Inventories", "Vegetation Resources Inventories", "Vegetation Resources Inventories")

#[42] Numerical. Specify scale (mapping scale or cell size) of product.
Original_scale<- c("NA", "1:20,000", "1:50,000", "1:50,000", "1:50,000", "1:50,000", "1:50,000", "1:20,000", "800x800m", "800x800m", "800x800m", "800x800m", "800x800m", "800x800m", "800x800m", "800x800m", "25x25m", "1:20,000", "1:20,000", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "30x30m", "25x25m", "25x25m", "25x25m", "100x100m", "100x100m", "100x100m", "100x100m", "100x100m", "1:20,000", "V1:20,000", "1:20,000", "1:20,000")
...
Year_represented<- c("Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "2022", "1991-2020", "1991-2020", "1991-2020", "1991-2020", "1991-2020", "1991-2020", "1991-2020", "1991-2020", "Unknown", "Unknown", "Unknown", "1985-2020", "2009", "2009", "2009", "2009", "2009", "2009", "2009", "2009", "2009", "2009", "2009", "2015", "2015", "2015", "2015", "2015", "2015", "2015", "2015", "2015", "2015", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "Unknown", "2005", "2005", "2005", "2005")

#[44] Categorical: Remote-sensed, Field-verified, VA of satellite imagery, Predictive modelling, Unknown.
Data_collection_method<- c("Field-verified", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "Field-verified", "Field-verified", "Field-verified", "Field-verified", "Field-verified", "Field-verified", "Field-verified", "Field-verified", "Field-verified", "Remote-sensed", "VA of satellite imagery", "VA of satellite imagery", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "Remote-sensed", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "VA of satellite imagery and Field verified", "VA of satellite imagery", "VA of satellite imagery", "VA of satellite imagery", "VA of satellite imagery")
...
Predictor_source <- as_tibble(Predictor_source)
...
kable(SHMPredictors_tab, caption = "Table 4: Enviromental Data Inputs")
```
...
kable(SHMModelAlgo_tab, caption = "Table 5: Model Algorithm and Validation")
...
Statistical_performance_just<- "Only a single metric was used, and AUC score was .70 which is considered below 'Low'... therefore not good."
...
if(All_inputs_valid) {
  ...
  } else if(identical(All_results, c("Low", "Low", "Low")) | Integrity_confidence == "N/A") {
...
kable(SHMModelQualityAssessment_tab, caption = "Table 6: Model Quality Assessment")
...
Insufficient_species_data_rationale <- "There appears to be a high number of points, yet not well distributed throughout its range."
...
Spatial_biases_species_data <- "Y"

#[74] If ‘Y’ to [73], provide details. Text. E.g., poor inventory in certain regions, inventories are road biased, etc.
Spatial_biases_species_data_description <- "Skink species input data are strongly tied to powerline corridors"
...
kable(SHMPresAbsGaps_tab, caption = "Table 7: Species Presence & Absence Data Gaps")


```


##SHM Gap Identification
#SHM Gap Identification - Environmental Predictors - related to section 4.2
...
Predictor_source_layer<- c("Terrestrial Ecosystem Mapping", "Terrestrial Ecosystem Mapping", "Terrestrial Ecosystem Mapping", "Terrestrial Ecosystem Mapping" , "Terrestrial Ecosystem Mapping", "Terrestrial Ecosystem Mapping", "Terrestrial Ecosystem Mapping", "Terrestrial Ecosystem Mapping", "Terrestrial Ecosystem Mapping", "Terrestrial Ecosystem Mapping", "Vegetation Resources Inventories", "Vegetation Resources Inventories", "Vegetation Resources Inventories", "Vegetation Resources Inventories", "Vegetation Resources Inventories", "Vegetation Resources Inventories", "Vegetation Resources Inventories", "2nd gen. Canadian Wetland Inventory", "2nd gen. Canadian Wetland Inventory", "Broad Ecosystem Inventory", "BC-DigitalGeo")
...
Associated_variable<- c("Surficial materials", "Landforms", "First decile ecosystem label - Percent cover of 'TA' talus slopes", "Stand compostion", "R/G/C", "Bioterrain codes", "Site modifiers", "Drainage", "Structural stage", "First decile ecosystem label - Distance to nearest 'CF' cultivated field", "Leading tree species", "Forest Age", "Crown closure", "Stems/hectare", "Percent herb cover", "Shrub height", "Percent shrub cover", "Distance to stream, river, lake, wetland", "Percent cover of water feature", "Ecosection", "Rock classes")

#[78] Categorical: Desired variable unavailable, Dataset inaccessible, Incomplete coverage, Poor resolution, Poor temporal alignment, Accuracy concerns
Predictor_source_layer <- as_tibble(Predictor_source_layer)
Issue <- Predictor_source_layer %>% 
  mutate(issue = case_when(
    ...
    str_detect(Predictor_source_layer$value, "2nd gen. Canadian Wetland Inventory") ~ "Dataset inaccessible",
...
Gap_description <- Predictor_source_layer %>% 
  mutate(gap_description = case_when(
    str_detect(Predictor_source_layer$value, "Terrestrial Ecosystem Mapping") ~ "TEM mapping for less than 50% of AOI; TEM mapping does not likely align with most skink records.",
    str_detect(Predictor_source_layer$value, "Vegetation Resources Inventories") ~ "VRI polygons covers almost 90% of AOI yet most of the attributes interested in are not available; experts say that VRI vintages prior to 2008 say they are not very accurate",
    str_detect(Predictor_source_layer$value, "2nd gen. Canadian Wetland Inventory") ~ "Wetland inventory database is presented in a peer-reviewed journal yet it is unclear how somebody downloads it, contacting primary author yields no response, prefer to just use Fresh Water Atlas",
    str_detect(Predictor_source_layer$value, "Broad Ecosystem Inventory") ~ "There is little variation in ecosection within skink range",
...
kable(SHMEnvgaps_tab, caption = "Table 8: Enviromental Data Gaps")
```
...
sheet = createSheet(wb, "PLSK_Model_Information")
...
saveWorkbook(wb, "Skink_Standard_Excel_Output.xlsx")
