...
update description: "Development of first version of script completed."
...
list.of.packages <- c("tidyverse", "readxl", "knitr", "lemon", "terra", "sf", "raster", "whitebox")
...
saga_tri_src_tif <- file.path(src_data_folder, r"(SAGA_GIS\bc_saga_terrain_ruggedness_index.tif)")
...
mat_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\MAT.tif)")
rh_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\RH.tif)")
...
```{r}
# Crop and mask SAGA TRI and DAH and write to tif
saga_tri_rast <- rast(saga_tri_src_tif)
saga_tri_rast <- crop(saga_tri_rast, ext(aoi_vect), mask=TRUE)
saga_tri_rast <- mask(saga_tri_rast, aoi_vect)
names(saga_tri_rast) <- "saga_tri"
saga_tri_nona_tif <- writeRaster(subst(saga_tri_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "saga_tri_noNA.tif"), overwrite=TRUE)
rm(saga_tri_rast, saga_tri_nona_tif)
...
mat_rast <- rast(mat_src_tif)
rh_rast <- rast(rh_src_tif)
...
mat_rast <- crop(mat_rast, ext(aoi_vect_prj) + .01)
rh_rast <- crop(rh_rast, ext(aoi_vect_prj) + .01)
...
mat_rast <- mask(mat_rast, aoi_vect_prj)
rh_rast <- mask(rh_rast, aoi_vect_prj)
...
mat_rast <- terra::project(mat_rast, crs(aoi_vect))
rh_rast <- terra::project(rh_rast, crs(aoi_vect))
...
mat_rast <- resample(mat_rast, dem_rast, method="near")
rh_rast <- resample(rh_rast, dem_rast, method="near")
...
names(mat_rast) <- "clim_mat"
names(rh_rast) <- "clim_rh"
...
mat_rast_nona_tif <- writeRaster(subst(mat_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_mat_noNA.tif"), overwrite=TRUE)
rh_rast_nona_tif <- writeRaster(subst(rh_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_rh_noNA.tif"), overwrite=TRUE)
...
rm(map_rast, mat_rast, rh_rast, shm_rast, tave_at_rast, tave_sm_rast, tave_sp_rast, tave_wt_rast, map_rast_nona_tif, mat_rast_nona_tif, rh_rast_nona_tif, shm_rast_nona_tif, tave_at_rast_nona_tif, tave_sm_rast_nona_tif, tave_sp_rast_nona_tif, tave_wt_rast_nona_tif)
```
