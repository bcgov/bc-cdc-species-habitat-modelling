---
title: "Coastal-tailed Frog Metadata and Quality Assessment Standard"
...
inputs: "001_SpeciesDataLoading_ASTR.Rmd"
        "002a_Environmental_predictors_loading_ASTR.Rmd"
        "002b_EnvironmentalDataLoading_ASTR.Rmd"
        "003_ASTR_ModelRunning.Rmd"

# List of outputs '[object name].[file type]' produced from this script; inputs often obtained from outputs of previous scripts
outputs: "ASTR_Standard_Excel_Output.xlsx"
         "ASTR_Standard_Word_Output.docx"
...
![Coastal-tailed Frog](ASTRphoto.png)
...
Project_area<-"Coastal-tailed frog range in British Columbia"

#[2] Numerical. Provide metric units (hectares, m2, km2).
#Area_size<- (sum(ASTR_range$SHAPE_Area))/1000000
Area_size<- "149644.3 km2"
...
Common_names<-"Coastal-tailed frog"
...
SHM_objective<-"Restoration decisions"
...
Access_date <- c("03-08-2023", "01-03-2023", "01-03-2023")
...
#Number_of_records <- c(nrow(ASTR_gbif), nrow(ASTR_EO),nrow(ASTR_WSI_SO))
Number_of_records <- c("175", "2706", "55")

#[22] Categorical: Citizen science, Targeted surveys, Expert knowledge, Various
Collection_method <- c("Various", "Various", "Various")
...
#Total_records_preclean <- sum(final_ASTR_point$OBSERVED_NUMBER)
Total_records_preclean <- "706"

#[27] DD-MM-YYYY; Earliest date of species observations. 

# min(ASTR_EO$OBSERVATION_MONTH)
# min(ASTR_EO$OBSERVATION_DAY)
# min(ASTR_EO$OBSERVATION_YEAR)
# 
# min(ASTR_WSI_SO$FIRST_OBS)

Date_range_min<- "03-1-1954"

#[27] DD-MM-YYYY; Latest date of species observations.
# max(ASTR_EO$OBSERVATION_MONTH)
# max(ASTR_EO$OBSERVATION_DAY)
# max(ASTR_EO$OBSERVATION_YEAR)
# 
# max(ASTR_WSI_SO$FIRST_OBS)

Date_range_max<- "12-31-2021"
...
#Final_records_postclean <- sum(final_ASTR_point$OBSERVED_NUMBER)
Final_records_postclean<-706
...
Predictor_short_nm <- tibble::as_tibble(list.files(path = "R:/22.0253_SHM_Year2/Worked_Examples/Coastal_Tailed_Frog/Data/Predictor_Inputs/Processed", pattern = '_noNA.tif$', all.files = TRUE, full.names=FALSE))
...
Predictor_full_nm<- c("Towns and cities", 
                      ...
                      "Abies_amabilis", "Chamaecyparis_nootkatensis", "Picea_engelmannii", "Picea_glauca",                                   "Picea_sitchensis", "Pinus_contorta", "Pinus_ponderosa", "Populus_tremuloides",                                        "Abies_lasiocarpa", "Pseudotsuga_menziesii", "Thuja_plicata", "Tsuga_heterophylla",                                    "Tsuga_mertensiana", "Acer_macrophyllum", "Alnus_rubra", 
...
Statistical_performance_just<- "Only a single metric was used, yet AUC score is .84 which is considered acceptable"
...
Gap_description <- Predictor_source_layer %>% 
  mutate(gap_description = case_when(
    str_detect(Predictor_source_layer$value, "Terrestrial Ecosystem Mapping") ~ "TEM mapping only avaialbe for 48.1% of AOI; mosaiced eras of TEM mapping does not likely align with most frog records.",
    str_detect(Predictor_source_layer$value, "Vegetation Resources Inventories") ~ "VRI polygons covers almost 90% of AOI yet most of the attributes interested in are not available; experts say that VRI vintages prior to 2008 are not very accurate",
    str_detect(Predictor_source_layer$value, "Broad Ecosystem Inventory") ~ "There is little variation in ecosection within frog range",
...
saveWorkbook(wb, "ASTR_Standard_Excel_Output.xlsx")
