...
update description: "Second version of script finalized: updated climate and SAGA predictor inputs."
output: pdf_document, .tif files
purpose: "Script will process source datasets into rasters aligned with DEM for input into calculations to determine environmental predictors for Pacific crabapple (Malus fusca) habitat."
...
raw_out_folder <- r"(R:\22.0253_SHM_Year2\Worked_Examples\Pacific_Crabapple\Data\Predictor_Inputs\Raw)"
proc_out_folder <- r"(R:\22.0253_SHM_Year2\Worked_Examples\Pacific_Crabapple\Data\Predictor_Inputs\Processed)"
dem_src_tif <- file.path(src_data_folder, r"(DEM_25m\bc_elevation_25m_bcalb.tif)")
saga_swi_src_tif <- file.path(src_data_folder, r"(SAGA_GIS\bc_saga_swi_mafu.tif)")
saga_dah_src_tif <- file.path(src_data_folder, r"(SAGA_GIS\bc_saga_diurnal_anisotropic_heating.tif)")
mwmt_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\MWMT.tif)")
mcmt_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\MCMT.tif)")
td_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\TD.tif)")
dd0_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\DD_0.tif)")
dd18_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\DD_18.tif)")
emt_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\EMT.tif)")
...
ahm_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\AHM.tif)")
pas_src_tif <- file.path(src_data_folder, r"(ClimateBC_normal_1991_2020\PAS.tif)")
...
aoi_sf = st_read(dsn=modeldata_fgdb, layer="PDROS13040_Malus_fusca")
...
mwmt_rast <- rast(mwmt_src_tif)
mcmt_rast <- rast(mcmt_src_tif)
td_rast <- rast(td_src_tif)
dd0_rast <- rast(dd0_src_tif)
dd18_rast <- rast(dd18_src_tif)
emt_rast <- rast(emt_src_tif)
map_rast <- rast(map_src_tif)
ppt_sm_rast <- rast(ppt_sm_src_tif)
ahm_rast <- rast(ahm_src_tif)
pas_rast <- rast(pas_src_tif)
...
mwmt_rast <- crop(mwmt_rast, ext(aoi_vect_prj) + .01)
mcmt_rast <- crop(mcmt_rast, ext(aoi_vect_prj) + .01)
td_rast <- crop(td_rast, ext(aoi_vect_prj) + .01)
dd0_rast <- crop(dd0_rast, ext(aoi_vect_prj) + .01)
dd18_rast <- crop(dd18_rast, ext(aoi_vect_prj) + .01)
emt_rast <- crop(emt_rast, ext(aoi_vect_prj) + .01)
map_rast <- crop(map_rast, ext(aoi_vect_prj) + .01)
ppt_sm_rast <- crop(ppt_sm_rast, ext(aoi_vect_prj) + .01)
ahm_rast <- crop(ahm_rast, ext(aoi_vect_prj) + .01)
pas_rast <- crop(pas_rast, ext(aoi_vect_prj) + .01)

# Mask each cropped raster to AOI's geometry
mwmt_rast <- mask(mwmt_rast, aoi_vect_prj)
mcmt_rast <- mask(mcmt_rast, aoi_vect_prj)
td_rast <- mask(td_rast, aoi_vect_prj)
dd0_rast <- mask(dd0_rast, aoi_vect_prj)
dd18_rast <- mask(dd18_rast, aoi_vect_prj)
emt_rast <- mask(emt_rast, aoi_vect_prj)
map_rast <- mask(map_rast, aoi_vect_prj)
ppt_sm_rast <- mask(ppt_sm_rast, aoi_vect_prj)
ahm_rast <- mask(ahm_rast, aoi_vect_prj)
pas_rast <- mask(pas_rast, aoi_vect_prj)

# Reproject each masked climate raster to Albers
mwmt_rast <- terra::project(mwmt_rast, crs(aoi_vect))
mcmt_rast <- terra::project(mcmt_rast, crs(aoi_vect))
td_rast <- terra::project(td_rast, crs(aoi_vect))
dd0_rast <- terra::project(dd0_rast, crs(aoi_vect))
dd18_rast <- terra::project(dd18_rast, crs(aoi_vect))
emt_rast <- terra::project(emt_rast, crs(aoi_vect))
map_rast <- terra::project(map_rast, crs(aoi_vect))
ppt_sm_rast <- terra::project(ppt_sm_rast, crs(aoi_vect))
ahm_rast <- terra::project(ahm_rast, crs(aoi_vect))
pas_rast <- terra::project(pas_rast, crs(aoi_vect))

# Resample each reprojected climate raster to align with DEM
mwmt_rast <- resample(mwmt_rast, dem_rast, method="near")
mcmt_rast <- resample(mcmt_rast, dem_rast, method="near")
td_rast <- resample(td_rast, dem_rast, method="near")
dd0_rast <- resample(dd0_rast, dem_rast, method="near")
dd18_rast <- resample(dd18_rast, dem_rast, method="near")
emt_rast <- resample(emt_rast, dem_rast, method="near")
map_rast <- resample(map_rast, dem_rast, method="near")
ppt_sm_rast <- resample(ppt_sm_rast, dem_rast, method="near")
ahm_rast <- resample(ahm_rast, dem_rast, method="near")
pas_rast <- resample(pas_rast, dem_rast, method="near")

# Assign names to values in each raster
names(mwmt_rast) <- "clim_mwmt"
names(mcmt_rast) <- "clim_mcmt"
names(td_rast) <- "clim_td"
names(dd0_rast) <- "clim_dd0"
names(dd18_rast) <- "clim_dd18"
names(emt_rast) <- "clim_emt"
names(map_rast) <- "clim_map"
names(ppt_sm_rast) <- "clim_ppt_sm"
names(ahm_rast) <- "clim_ahm"
names(pas_rast) <- "clim_pas"

# Write each resampled noNA climate raster to processed output folder
mwmt_rast_nona_tif <- writeRaster(subst(mwmt_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_mwmt_noNA.tif"), overwrite=TRUE)
mcmt_rast_nona_tif <- writeRaster(subst(mcmt_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_mcmt_noNA.tif"), overwrite=TRUE)
td_rast_nona_tif <- writeRaster(subst(td_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_td_noNA.tif"), overwrite=TRUE)
dd0_rast_nona_tif <- writeRaster(subst(dd0_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_dd0_noNA.tif"), overwrite=TRUE)
dd18_rast_nona_tif <- writeRaster(subst(dd18_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_dd18_noNA.tif"), overwrite=TRUE)
emt_rast_nona_tif <- writeRaster(subst(emt_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_emt_noNA.tif"), overwrite=TRUE)
...
ahm_rast_nona_tif <- writeRaster(subst(ahm_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_ahm_noNA.tif"), overwrite=TRUE)
pas_rast_nona_tif <- writeRaster(subst(pas_rast, NA, 0, others=NULL, raw=TRUE), file.path(proc_out_folder, "clim_pas_noNA.tif"), overwrite=TRUE)

rm(mwmt_rast, mcmt_rast, td_rast, dd0_rast, dd18_rast, emt_rast, map_rast, ppt_sm_rast, ahm_rast, pas_rast, mwmt_rast_nona_tif, mcmt_rast_nona_tif, td_rast_nona_tif, dd0_rast_nona_tif, dd18_rast_nona_tif, emt_rast_nona_tif, map_rast_nona_tif, ppt_sm_rast_nona_tif, ahm_rast_nona_tif, pas_rast_nona_tif)
...
# Mask to AOI's geometry
ntems_fire_rast <- mask(ntems_fire_rast, aoi_vect)
...
# Rasterize
ccl_rast <- rasterize(ccl_vect, dem_rast, fun=1, background=0)

# Mask to AOI's geometry
ccl_rast <- mask(ccl_rast, aoi_vect)

# Write to .tif - whitebox tools require a .tif file path as input
ccl_dist_input_tif <- writeRaster(ccl_rast, file.path(proc_out_folder, "ccl_dist_input.tif"), overwrite=TRUE)
rm(ccl_vect, ccl_query, ccl_rast, ccl_dist_input_tif)
...
#str_query <- paste("select FEATURE_SOURCE, STREAM_ORDER from ", fwa_str_src_fc, " where FEATURE_SOURCE NOT IN ('linear stream -', 'RiverStream', 'ditch',  'areal stream sk') AND STREAM_ORDER > 1")
str_query <- paste("select STREAM_ORDER from ", fwa_str_src_fc, " where STREAM_ORDER > 1")
fwa_str_vect <- vect(st_read(dsn=fwa_src_fgdb, query=str_query))
...
rm(fwa_one_rast, fwa_dist_input_tif)
...
# Rasterize
dra_rast <- rasterize(dra_vect, dem_rast, fun=1, background=0)
rm(dra_vect)

# Mask to AOI's geometry
dra_rast <- mask(dra_rast, aoi_vect)

# Write to .tif - whitebox tools require a .tif file path as input
dra_dist_input_tif <- writeRaster(dra_rast, file.path(proc_out_folder, "dra_dist_input.tif"), overwrite=TRUE)
rm(dra_rast)
...
# Truncate to integer values (distance values don't need to be more precise than 1m)
...
# Rasterize
admin_rast <- rasterize(admin_res, dem_rast, fun=1, background=0)

# Mask to AOI's geometry
admin_rast <- mask(admin_rast, aoi_vect)

# Rasterize and mask to AOI's geometry - whitebox tools require a .tif file path as input
admin_dist_input_tif <- writeRaster(admin_rast, file.path(proc_out_folder, "admin_dist_input.tif"), overwrite=TRUE)
rm(admin_vect, admin_df, admin_qry, admin_res, admin_rast, admin_dist_input_tif)
...
rm(admin_dist_rast, admin_dist_nona_tif, x)
