---
title: "Presence & Absence Data Loading"
...
setwd("R:/22.0253_SHM_Year2/Worked_Examples/Western_Skink")
...
PLSK_range <- sf::st_read(fgdb, layer = fc_list[[1]])
PLSK_EO <- sf::st_read(fgdb, layer = fc_list[[3]])
PLSK_EO_Ext <- sf::st_read(fgdb, layer = fc_list[[4]])
PLSK_WSI_SO <- sf::st_read(fgdb, layer = fc_list[[5]])
PLSK_iNat_obsc <- sf::st_read(fgdb, layer = fc_list[[6]])
...
gbif_data <- occ_data(scientificName = Current_scientific_name, hasCoordinate = TRUE, limit = 20000, decimalLongitude = "-124, -114", decimalLatitude = "49, 50")
...
ggplot() +
  geom_sf(data = PLSK_range) +
  geom_sf(data = PLSK_EO_Ext) +
  geom_sf(data = PLSK_EO, color = "blue", size = 1) +
  geom_sf(data = PLSK_WSI_SO, color = "green") +
...
and they overlap well with the species occurrence information. They
don't appear to overlap with GBIF data. There are multiple options to
move forward here. I carry out two options here. First, I take the
easiest path and join the points and ignore the polygons. Removing them
for the sake of simplicity and consistency. Second, intersect points
from WSI_SO with EO polygons, then combine the GBIF and SO/EO data into
one SF. Another approach may be to include a weighting for the EO
polygons based on the number/density of SO within the polygon. However,
this requires a deeper dive into the existing data to ensure that
sampling bias does not impacting any weighting, and that is an involved
process.
...
PLSK_point_map$OBSERVED_NUMBER[is.na(PLSK_point_map$OBSERVED_NUMBER)] = 1

PLSK_point_raster <- st_rasterize(PLSK_point_map)#Stars object - change to sf:
PLSK_point_raster <- st_as_sf(PLSK_point_raster)
```

## Generate Background or Pseudoabsence Points:

```{r}
PLSK_range <- st_rasterize(PLSK_range %>% dplyr::select(SHAPE))
write_stars(PLSK_range, "PLSK_range.tif")
PLSK_range_rast <- raster("PLSK_range.tif")

background_points <- dismo::randomPoints(PLSK_range_rast,1500)
bg_points <- background_points %>%
  as.data.frame %>%
  sf::st_as_sf(coords = c(1,2))
bg_points$SURVEY_OBSERVATION_ID <- c(101:1600)

#Then we need them to link up to known points:
bg_points <- st_set_crs(bg_points, "EPSG:3005")
bg_points <- dplyr::rename(bg_points, GEOMETRY = geometry)

bg_points[setdiff(names(PLSK_point_map), names(bg_points))] <- NA
bg_points$OBSERVED_NUMBER[is.na(bg_points$OBSERVED_NUMBER)] = 0

PLSK_points <- rbind(PLSK_point_map,bg_points)
keepers <- c("GEOMETRY", "OBSERVED_NUMBER", "SURVEY_OBSERVATION_ID","UTM_EASTING", "UTM_NORTHING", "LATITUDE","LONGITUDE")
PLSK_points_clean <- PLSK_points[,which(names(PLSK_points) %in% keepers)]
PLSK_points_clean$Obs <- ifelse(PLSK_points_clean$OBSERVED_NUMBER == 0, 0, 1)
```
